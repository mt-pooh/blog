...
AgentCore 標準の認可機構は JWT のクレームしか参照できません。「ロールを変更したら即座に反映する」という要件を満たすには、DB に置いたロールデータをリクエストごとに参照する必要がありますが、それができない設計でした。結果として、認可ロジックは自前で実装することにしました。
...
3 つの壁を踏まえた結論は「AgentCore だけでは要件を満たせない」です。グループ全体でセキュアに AI をスケールさせていくには、内製以外の選択肢はないと判断しました。
...
「爆発半径を縮小・長期シークレットを持たない・ユーザーの同意を得る」の 3 要件を同時に満たせる方式がなく、かなり悩みました。最終的に採用したのが下記の方式です。
...
壁 3 で触れたとおり、CloudWatch Logs Insights はコストとマルチテナント分離の面で要件を満たせなかったため、Aurora ETL パターンを採用しています。SQL の柔軟な全文検索と RLS によるテナント分離を両立できる構成です。
...
特に「日本特有の監査・稟議要件」は、金融機関を含む日本のエンタープライズ企業への展開を見据えると最重要課題です。J-SOX 対応の証跡管理・多段階承認フロー・閉域ネットワーク対応など、グローバルの大手ベンダーが構造的に後回しにしやすい要件を、AI Connect Hub が担うべき領域だと思っています。MCP エコシステム自体もまだ急速に進化しており、仕様の追従と並行して開発を続けています。

---

## 付録: エンタープライズで MCP を動かし続ける理由

「MCP は不要、CLI で十分」という意見があります。
...
Finatext グループでは「[vibe working](https://note.com/fozzhey/n/nc7b681faabf6)」という取り組みで、非エンジニアも含めたグループ全体での AI エージェント活用を推進しています。データアナリストやビジネスサイドのメンバーが AI エージェントを通じて Jira を操作したり、Notion にまとめたりする世界を目指しています。こうした非エンジニアに「CLI を覚えてください」は流石に無理があります。
...
│                    管理層（プロトコル非依存）                       │
│                                                                 │
│   カタログ管理 / RBAC / 監査ログ / ユーザー管理 / IdP 連携           │
│   フロントエンド全体 / 申請・承認ワークフロー                         │
│                                                                 │
│   ← MCP という言葉を知らない。「ツール」「ゲートウェイ」で考える →     │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│              実行アダプター層（プロトコル依存）                      │
│                                                                 │
│   AgentCoreClientImpl / MCP Proxy コンテナ / Proxy Lambda       │
│                                                                 │
│   ← 現時点では MCP / AgentCore に固有な実装 →                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
