...
接続先サービスごとに認証方式が異なることは Layer 3 で述べたとおりです。ここでは設計の難易度が高かった Google サービスの認証を例に、影響範囲をどう抑えるかを具体的に説明します。

Google サービスへの認証で問題になるのは「AI に人間と同等の権限を渡してしまう」点です。代表的な方式を検討しましたが、いずれも影響範囲が大きすぎました。
...
「影響範囲を縮小・長期シークレットを持たない・ユーザーの同意を得る」の 3 要件を同時に満たせる方式がなく、かなり悩みました。最終的に採用したのが下記の方式です。
