...
---
## この記事について
SnowVillageで行われている[Apache Iceberg: The Definitive Guide \[Book\]](https://www.oreilly.com/library/view/apache-iceberg-the/9781098148614/)の輪読会用にChapter5 Iceberg Catalogsの内容をまとめたものです。
...
### RESTカタログ
>  RESTカタログは、その名の通り、RESTfulサービスプロバイダーをIcebergカタログとして利用します。このアプローチは、RESTカタログが特定の実装ではなくインターフェースであるため、**他の多くのカタログとは異なり、柔軟性が高いという興味深い特徴**があります。RESTカタログインターフェースを実装するサービスは、テーブルのパスを現在のメタデータファイルにマッピングする方法を自由に選択できます。実際には、このセクションで述べた他のカタログにそのデータを保存することも可能です。
