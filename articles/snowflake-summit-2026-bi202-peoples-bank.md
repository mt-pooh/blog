...
ここがスライドの綺麗な 3 分類とトランスクリプトの差分で、個人的にいちばん刺さったポイントです。実際にはコアの 3 システムどころではなく、

- リース、プレミアムファイナンス、保険、信託／投資 など複数の事業ライン
- Jack Henry、Fiserv、Meridian Lake、Compass、各種リース／フランチャイズ系システム
- 合計でおよそ **20 個のサイロ**
...
トランスクリプト側では、より具体的に「レポートが事業チームに届くまで **2〜3 週間**かかり、待ちきれず手作業で抜き出す → 精度・リスクの懸念が増える」という悪循環が語られていました。
...
> 教訓:いきなり理想形を作ろうとせず、「暫定の Golden ID で現状を可視化 → ソースを棚卸し → 本格基盤の設計に反映」という段階移行を取っている。データ基盤刷新あるあるの「ビッグバン移行で死ぬ」を回避している点が好印象でした。
...
「AI の強さは、その下のファウンデーションの強さに等しい」。Summit で一番地味で、一番本質的なセッションでした。
